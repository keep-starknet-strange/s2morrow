...
use crate::params_128s::{
    HashOutput, SPX_WOTS_LEN, deserialize_hash_output_array, serialize_hash_output_span,
};
...
pub impl WotsSignatureSerde of Serde<WotsSignature> {
    fn serialize(self: @WotsSignature, ref output: Array<felt252>) {
        serialize_hash_output_span(self.span(), ref output);
    }

    fn deserialize(ref serialized: Span<felt252>) -> Option<WotsSignature> {
        let arr = deserialize_hash_output_array(ref serialized);
