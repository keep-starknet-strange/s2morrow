...
use crate::sha2::{sha256_inc_finalize, Sha256State};
use crate::word_array::{WordSpan, WordSpanTrait, WordArray, WordArrayTrait};
...
pub fn thash_128s(mut data: WordSpan) -> HashOutput {
    ...
    let [d0, d1, d2, d3, _, _, _, _] = sha256_inc_finalize(ref state, input.into(), last_word, last_word_len);
    [d0, d1, d2, d3]
}
...
mod tests {
    ...
    #[test]
    fn test_thash_128s() {
        let data = array![12061, 501484376, 3892510720, 33095680, 0, 3662217216, 0, 0, 3513816658, 740138468, 3821323496, 1704729478];
